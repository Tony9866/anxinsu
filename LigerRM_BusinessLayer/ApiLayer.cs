...
using SignetInternet_BusinessLayer.Models.Rent_Rent;
...
namespace SignetInternet_BusinessLayer
{
    public class ApiLayer : BaseHelper
    {
        ...
        public string GetCity(string ProvincesId)
        {
            ...
            {
                StringBuilder str = new StringBuilder();
                str.Append("SELECT * FROM Public_City WHERE Public_City.provinceid='").Append(ProvincesId).Append("'");
                List<ProvincesModel> list = GetList<ProvincesModel>(str.ToString());
                if (list == null)
                {
                    list = new List<ProvincesModel>();
        ...
        public string GetArea(string CityId)
        {
            ...
            {
                StringBuilder str = new StringBuilder();
                str.Append("SELECT * FROM Public_Area WHERE Public_Area.cityid='").Append(CityId).Append("'");
                List<ProvincesModel> list = GetList<ProvincesModel>(str.ToString());
                if (list == null)
                {
                    list = new List<ProvincesModel>();
        ...
        public List<TwoRent_BasicAttributes> BasicAttributes()
        {
            try
            {
                StringBuilder str = new StringBuilder();
                str.Append("SELECT * FROM Rent_BasicAttributes WHERE Type='1'");
                List<TwoRent_BasicAttributes> list = GetList<TwoRent_BasicAttributes>(str.ToString());
                return list;

            }
            catch (Exception ex)
            {
                return new List<TwoRent_BasicAttributes>();
            }
        }

        public List<Rent_BasicAttributes> BasicAttributes(int Type)
        {
            try
            {
                StringBuilder str = new StringBuilder();
                str.Append("SELECT * FROM Rent_BasicAttributes WHERE Type='").Append(Type).Append("'");
                List<Rent_BasicAttributes> list = GetList<Rent_BasicAttributes>(str.ToString());
                return list;
            }
            catch (Exception ex)
            {
                return new List<Rent_BasicAttributes>();
            }
        }
        ...
        public void Initials()
        {
            ...
            {
                string Char = GetPYChar(item.province).ToUpper();
                string String = GetPYString(item.province).ToUpper();
                string py = convertCh(item.province).ToLower();
                string sql = "update Public_Provinces set Initials='" + Char + "',Abbreviation='" + String + "',Pinyin='" + py + "' where provinceid=" + item.provinceid;
                MySQLHelper.ExecuteNonQuery(MySQLHelper.SqlConnString, MySQLHelper.CreateCommand(sql.ToString()));

            }
        }

        public void Initials2()
        {
            StringBuilder str = new StringBuilder();
            str.Append("SELECT * FROM Public_City");
            List<CityModel> list = GetList<CityModel>(str.ToString());
            foreach (var item in list)
            {
                string Char = GetPYChar(item.city).ToUpper();
                string String = GetPYString(item.city).ToUpper();
                string py = convertCh(item.city).ToLower();
                string sql = "update Public_City set Initials='" + Char + "',Abbreviation='" + String + "',Pinyin='" + py + "' where cityId=" + item.cityId;
                MySQLHelper.ExecuteNonQuery(MySQLHelper.SqlConnString, MySQLHelper.CreateCommand(sql.ToString()));

            }
        }

        public void Initials3()
        {
            StringBuilder str = new StringBuilder();
            str.Append("SELECT * FROM Public_Area ");
            List<AreaModel> list = GetList<AreaModel>(str.ToString());
            foreach (var item in list)
            {
                string Char = GetPYChar(item.area).ToUpper();
                string String = GetPYString(item.area).ToUpper();
                string py = convertCh(item.area).ToLower();
                string sql = "update Public_Area set Initials='" + Char + "',Abbreviation='" + String + "',Pinyin='" + py + "' where areaid=" + item.areaid;
                MySQLHelper.ExecuteNonQuery(MySQLHelper.SqlConnString, MySQLHelper.CreateCommand(sql.ToString()));
