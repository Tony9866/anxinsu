      ...
      $("#formsearch").ligerForm({
          ...
          { display: "使用单位名称", name: "co_corp_name", newline: false, labelWidth: 100, width: 120, space: 20, type: "text", cssClass: "field" },
                    { display: "所属区域", name: "所属区域", newline: false, labelWidth: 80, width: 200, space: 20, type: "select", cssClass: "field",
                                             options: {
                            url: '../handler/select.ashx?view=t_area&idfield=ar_area_id&textfield=ar_area_name&distinct=true&needAll=1',
                            initValue: '全部', valueFieldID: 'co_area_id', selectBoxWidth: '200'
                        }
                     },
                    { display: "企业类型", name: "企业类型", newline: false, labelWidth: 80, width: 160, space: 20, type: "select", cssClass: "field",
                        options: {
      ...
      function toolbarBtnItemClick(item) {
          switch (item.id) {
              case "add":
                  ...
                  wopen('CorporationDetail.aspx?CorpId=&type=E', '使用单位详细信息', '610', '550');
                  break;
              case "view":
              ...
              case "modify":
                  ...
                  LG.ajax({
                      ...
                      data: { sourceStr: selected.co_corp_id },
                      success: function (data) {
                          wopen('../InfoRegister/CorporationDetail.aspx?CorpId=' + data + '&type=E', '使用单位详细信息', '610', '550');
                      },
                      error: function (message) {
                      }
                  });
