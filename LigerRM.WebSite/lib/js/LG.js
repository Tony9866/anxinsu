(function ($) {
    ...
    LG.bulidFilterGroup = function (form) {
        ...
        .each(function () {
            ...
            if (!this.name) return;
            if (inputType != "hidden" && !$(this).hasClass("field")) return;
            if (inputType != "hidden" && ($(this).val() == null || $(this).val() == "")) return;
    ...
    LG.appendSearchButtonsNoAdvance = function (form, grid) {
        if (!form) return;
        form = $(form);
        //        //搜索按钮 附加到第一个li  高级搜索按钮附加到 第二个li
        var container = $('<ul><li style="margin-right:8px"></li><li></li></ul><div class="l-clear"></div>').appendTo(form);
        LG.addSearchButtonsNoAdvance(form, grid, container.find("li:eq(0)"));
    };
    ...
    //创建表单搜索按钮：搜索、高级搜索
    LG.addSearchButtonsNoAdvance = function (form, grid, btn1Container) {
        if (!form) return;
        if (btn1Container) {
            LG.createButton({
                appendTo: btn1Container,
