using System;

/**
* 商户基础 配置
* @author guoyx e-mail:guoyx@lianlian.com
* @date:2013-6-25 下午01:45:40
* @version :1.0
*
*/

namespace LigerRM.Common.Payment
{
	public class PartnerConfig
	{

		// RSA银通公钥
		public static string YT_PUB_KEY     = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCSS/DiwdCf/aZsxxcacDnooGph3d2JOj5GXWi+q3gznZauZjkNP8SKl3J2liP0O6rU/Y/29+IUe+GTMhMOFJuZm1htAtKiu5ekW0GlBMWxf4FPkYlQkPE0FtaoMP3gYfh+OwI+fIRrpW3ySn3mScnc6Z700nU/VYrRkfcSCbSnRwIDAQAB";
		// RSA商户私钥
		public static string TRADER_PRI_KEY = "";
		// MD5 KEY
		public static string MD5_KEY        = "201408071000001543test_20140812";
		// 接收异步通知地址
        public static string NOTIFY_URL = "http://localhost:55991/Payment/NotifyUrl.aspx";  //请变更yourdomain为你的域名（及端口）
		// 支付结束后返回地址
        public static string URL_RETURN = "http://localhost:55991/Payment/ReturnUrl.aspx?OrderId={0}";    //请变更为您的返回地址
		// 商户编号
        public static string OID_PARTNER = "201705091001717481";     //请变更为您的商户号
		// 签名方式 RSA或MD5
		public static string            SIGN_TYPE="RSA";    					//请选择签名方式
		// 接口版本号，固定1.0
		public static string VERSION        = "1.0";
		// 业务类型，连连支付根据商户业务为商户开设的业务类型； （101001：虚拟商品销售、109001：实物商品销售）
		public static string BUSI_PARTNER   = "101001";   //请选择业务类型
	}
}

